...
function matlabReusePowerShellContent(
  ...
  return [
    ...
    '$hasAnyMatlabProcess = $matlabProcesses.Count -gt 0',
    ...waitLines,
    'function Invoke-NeuroPredictMatlabCommands($matlab) {',
    '  try { $matlab.Visible = 1 } catch { }',
    '  $matlab.Execute($matlabCommands) | Out-Null',
    '}',
    'function Start-NeuroPredictVisibleMatlab {',
    '  if ($hasAnyMatlabProcess) {',
    '    Write-Warning "Starting a new visible MATLAB/EEGLAB window because the running MATLAB process could not be activated or automated."',
    '  }',
    ...
    '  } catch {',
    '    Start-NeuroPredictVisibleMatlab',
    '  }',
    '}',
    ...waitLoop,
    '',
...
function matlabEntryScriptContents(): string {
  return [
    ...
    "        fprintf('Final preprocessed EEG already exists: %s\\n', finalPath);",
    '        continue;',
    '    end',
    "    if exist(stage04Path, 'file') == 2",
    "        fprintf('Resuming final rereference from ICA artifact file: %s\\n', stage04Path);",
    ...
    '        continue;',
    '    end',
    "    if exist(stage03Path, 'file') == 2",
    "        fprintf('ICA-ready EEG already exists and is waiting for manual artifact rejection: %s\\n', stage03Path);",
    '        continue;',
    '    end',
    "    if exist(stage02Path, 'file') == 2",
    "        fprintf('Resuming bad-channel interpolation and ICA from bad-segment file: %s\\n', stage02Path);",
    ...
    '        continue;',
    '    end',
    "    if exist(stage01Path, 'file') == 2",
    "        fprintf('Bad-segment EEG already exists and is waiting for manual rejection: %s\\n', stage01Path);",
    '        continue;',
    '    end',
...
function writeMatlabExecutionPowerShellLauncher(
  ...
  const content = [
    "$ErrorActionPreference = 'Stop'",
    `$matlabExe = ${powershellSingleQuoted(matlabExecutable)}`,
    `$matlabCommands = ${powershellSingleQuoted(buildMatlabRunExpression(launcherScriptPath))}`,
    `$donePath = ${powershellSingleQuoted(donePath)}`,
    `$errorPath = ${powershellSingleQuoted(errorPath)}`,
    `$logPath = ${powershellSingleQuoted(logPath)}`,
    '$startupTimeoutAt = (Get-Date).AddSeconds(180)',
    '$timeoutAt = (Get-Date).AddSeconds(14400)',
    "Remove-Item -LiteralPath $donePath, $errorPath -Force -ErrorAction SilentlyContinue",
    "$matlabArgs = @('-nosplash', '-wait', '-r', $matlabCommands)",
    '$process = Start-Process -FilePath $matlabExe -ArgumentList $matlabArgs -PassThru',
    'while ((Get-Date) -lt $startupTimeoutAt) {',
    '  if ((Test-Path -LiteralPath $logPath) -or (Test-Path -LiteralPath $donePath) -or (Test-Path -LiteralPath $errorPath)) { break }',
    '  if ($process.HasExited) { break }',
    '  Start-Sleep -Seconds 1',
    '}',
    'if (-not (Test-Path -LiteralPath $logPath) -and -not (Test-Path -LiteralPath $donePath) -and -not (Test-Path -LiteralPath $errorPath)) {',
    '  if (-not $process.HasExited) { Stop-Process -Id $process.Id -Force -ErrorAction SilentlyContinue }',
    '  Write-Error "MATLAB did not start executing the NeuroPredict script within 180 seconds. No log, done, or error file was created."',
    '  exit 1',
    '}',
    'while ((Get-Date) -lt $timeoutAt) {',
    '  if (Test-Path -LiteralPath $donePath) {',
    ...
    '    if (Test-Path -LiteralPath $logPath) { Get-Content -LiteralPath $logPath -ErrorAction SilentlyContinue }',
    '    Write-Error "MATLAB exited before NeuroPredict status files were written."',
    '    exit 1',
    '  }',
    '  Start-Sleep -Seconds 2',
    '}',
